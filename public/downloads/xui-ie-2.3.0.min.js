...
d){return this.each(function(f){for(var h=I(f),m=O(h,c),k=m.length;k--;)if(d===B||d.guid===m[k].guid){f.detachEvent("on"+c,m[k]);W(H[h][c],k,1)}H[h][c].length===0&&delete H[h][c];for(var r in H[h])return;delete H[h]})},fire:function(c,d){return this.each(function(f){if(f==t&&!f.fireEvent)f=t.documentElement;var h=t.createEventObject();h.data=d||{};h.eventName=c;ha.indexOf(c)>-1?f.fireEvent("on"+c,h):O(I(f),c).forEach(function(m){m.call(f)})})}});S.forEach(function(c){v.fn[c]=function(d){return function(f){return f?
this.on(d,f):this.fire(d)}}(c)});v.ready=function(c){domReady(c)};I.id=1;var da=/^(\s|\u00A0)+|(\s|\u00A0)+$/g;v.extend({setStyle:function(c,d){c=X(c);return this.each(function(f){f.style[c]=d})},getStyle:function(c,d){if(d===B){var f=[];this.each(function(h){f.push(T(h,c))});return f}else return this.each(function(h){d(T(h,c))})},addClass:function(c){return this.each(function(d){if(P(d,c)===false)d.className=R(d.className+" "+c)})},hasClass:function(c,d){var f=this;return this.length&&function(){var h=
true;f.each(function(m){if(P(m,c))d&&d(m);else h=false});return h}()},removeClass:function(c){if(c===B)this.each(function(f){f.className=""});else{var d=Q(c);this.each(function(f){f.className=f.className.replace(d,"$1")})}return this},toggleClass:function(c){return this.each(function(d){d.className=P(d,c)?R(d.className.replace(Q(c),"$1")):R(d.className+" "+c)})},css:function(c){for(var d in c)this.setStyle(d,c[d]);return this}});var U={},Q=function(c){var d=U[c];if(!d){d=new RegExp("(^|\\s+)"+c+"(?:\\s+|$)");
U[c]=d}return d};(function(c,d){function f(q,y,w){return(q+(y-q)*w).toFixed(3)}function h(q,y,w){return q.substr(y,w||1)}function m(q,y,w){for(var x=2,z,u,C=[],a=[];z=3,u=arguments[x-1],x--;)if(h(u,0)=="r")for(u=u.match(/\d+/g);z--;)C.push(~~u[z]);else{if(u.length==4)u="#"+h(u,1)+h(u,1)+h(u,2)+h(u,2)+h(u,3)+h(u,3);for(;z--;)C.push(parseInt(h(u,1+z*2,2),16))}for(;z--;){x=~~(C[z+3]+(C[z]-C[z+3])*w);a.push(x<0?0:x>255?255:x)}return"rgb("+a.join(",")+")"}function k(q){var y=parseFloat(q);q=q.replace(/^[\-\d\.]+/,
"");return isNaN(y)?{v:q,f:m,u:""}:{v:y,f:f,u:q}}function r(q){var y={},w=o.length,x;l.innerHTML='<div style="'+q+'"></div>';for(q=l.childNodes[0].style;w--;)if(x=q[o[w]])y[o[w]]=k(x);return y}var l=t.createElement("div"),o="backgroundColor borderBottomColor borderBottomWidth borderLeftColor borderLeftWidth borderRightColor borderRightWidth borderSpacing borderTopColor borderTopWidth bottom color fontSize fontWeight height left letterSpacing lineHeight marginBottom marginLeft marginRight marginTop maxHeight maxWidth minHeight minWidth opacity outlineColor outlineOffset outlineWidth paddingBottom paddingLeft paddingRight paddingTop right textIndent top width wordSpacing zIndex".split(" ");
...
e,g){for(var j=a,i=[],p=b,n,s,L=b&&b[0]&&l.isXML(b[0]);a&&b.length;){for(var E in o.filter)if((n=o.leftMatch[E].exec(a))!=null&&n[2]){var K=o.filter[E],F,A;A=n[1];s=false;n.splice(1,1);if(A.substr(A.length-1)!=="\\"){if(p===i)i=[];if(o.preFilter[E])if(n=o.preFilter[E](n,p,e,i,g,L)){if(n===true)continue}else s=F=true;if(n)for(var N=0;(A=p[N])!=null;N++)if(A){F=K(A,n,N,p);var V=g^!!F;if(e&&F!=null)if(V)s=true;else p[N]=false;else if(V){i.push(A);s=true}}if(F!==B){e||(p=i);a=a.replace(o.match[E],"");
